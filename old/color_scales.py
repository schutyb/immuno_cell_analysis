...
phasor-derived phase information.

Two colormap families are currently included:

- ``spectral``:
    Full inverted spectrum, designed for broad phase/lifetime coverage.

- ``reds_to_greens``:
    A red-to-green scale intended for green-channel FLIM visualization,
    where a full rainbow is not desired.
...
ScaleName = Literal["spectral", "reds_to_greens"]
...
def make_reds_to_greens_colormap(n: int = 2048) -> LinearSegmentedColormap:
    ...
    This scale is intended for green-channel FLIM visualization, where a
...
def get_phase_colormap(
    ...
    Parameters
    ----------
    name : {"spectral", "reds_to_greens"}, default="spectral"
...
def phase_to_rgb(
    ...
    scale : {"spectral", "reds_to_greens"}, default="spectral"
