...
def collect_cases(root_dir: Path, patient_filter: Optional[str] = None) -> List[Case]:
    ...
    for phasor_path in root_dir.rglob("phasor_corr.tif"):
...
def read_phasor_gs(phasor_path: Path) -> Tuple[np.ndarray, np.ndarray]:
    """
    Reads first-harmonic G and S from phasor_corr.tif
...
def roi_table_from_case(case: Case) -> pd.DataFrame:
    """
    One ROI -> one corrected phasor point (g_mean, s_mean)
    """
    ...
    for prop in props:
        ...
        rows.append({
            ...
            "visit": case.visit,
            "phasor_corr_path": str(case.phasor_path),
...
def plot_visit_gmm(df_visit: pd.DataFrame, cluster_df: pd.DataFrame, outpath: Path) -> None:
    ...
    for bio_label in ["elastin", "cells", "melanin"]:
        ...
        plot_phasor(
            ...
            ax=ax,
            title=f"ROI phasor GMM (corrected) - {df_visit['visit'].iloc[0]}",
            show=False,
        )
        plotted_any = True
...
def main() -> None:
    ...
    if not cases:
        raise RuntimeError("No encontré ningún phasor_corr.tif con máscara asociada.")
    ...
    roi_points_csv = OUTPUT_DIR / "roi_phasor_points_all_corrected.csv"
    ...
    for visit in sorted(df_all_rois["visit"].unique()):
        ...
        plot_path = OUTPUT_DIR / f"{visit}_roi_phasor_gmm_corrected.png"
    ...
    if all_cluster_stats:
        ...
        cluster_csv = OUTPUT_DIR / "phasor_cluster_stats_by_visit_corrected.csv"
    ...
    if all_roi_labeled:
        ...
        roi_labeled_csv = OUTPUT_DIR / "roi_phasor_points_with_gmm_labels_corrected.csv"
